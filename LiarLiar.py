...
class arePantsonFire(nn.Module):
    ...
    def forward(self, sentence, justification, credit_history): # 1 Marks
        ...
        position_output= self.position_feedforward.forward(attention_output)
        first_conv_output= self.first_conv(position_output)
        first_conv_output= nn.functional.relu(first_conv_output)
        flatten_conv_output= self.flatten_conv(first_conv_output)
        flat_func= nn.Flatten()
        flatten_output= flat_func(flatten_conv_output)
        linear1_output= self.linear1(flatten_output)
        linear2_output= self.linear2(linear1_output)
        bilinear_output= self.bilinear(linear2_output,credit_history)
        classifier_output= self.classifier(bilinear_output)
    ...
    def get_convolutions(self, input_dim, hidden_dim): # 0.5 Marks
        ...
        first_conv= nn.Conv1d(in_channels=upscale_conv.out_channels, out_channels=upscale_conv.out_channels//2, kernel_size=3, stride=1, padding=1)
        flatten_conv= nn.Conv1d(in_channels=first_conv.out_channels, out_channels=1, kernel_size=5, stride=1, padding=2)
    ...
    def get_linears_layers(self, max_length_sentence): # 0.5 Marks
        ...
        linear2= nn.Linear(in_features=linear1.out_features,out_features=6)
        bilinear= nn.Bilinear(in1_features=linear2.out_features,in2_features=5,out_features=12,bias=True)
