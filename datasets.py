...
class dataset(Dataset):
	...
	def __getitem__(self, idx): # 1.25 Mark
		data_dict = {}
		stat=self.dataframe.iloc[idx][3].lower()
		just=self.dataframe.iloc[idx][15].lower()
		statement=word_tokenize(stat)
		justification=word_tokenize(just)
		...
		for pos, word in enumerate(justification):
			try:
				...
				print('Word not in Vocab. Placing it at origin')
				continue
		vectorized_statement=torch.from_numpy(vectorized_statement)
		vectorized_justification=torch.from_numpy(vectorized_justification)
		temp=self.dataframe.iloc[idx][9:14].values
		credit_history=torch.from_numpy(numpy.asarray(temp[[3,2,0,1,4]],"float64"))
		...
		data_dict["statement"]=vectorized_statement.float()
		data_dict["justification"]=vectorized_justification.float()
		data_dict["label"]=label
		data_dict["credit_history"]=credit_history.float()
